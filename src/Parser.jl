...
function parse_return!(parser::Parser)::Union{ReturnNode,Error}
    ...
    advance!(parser).type == EOL && return ReturnNode(
        ...
        NoneNode(Range(return_token.position.end_position, return_token.position.end_position)),
    )
    value = parse_keywords!(parser)
    isa(value, Error) && return value
    return ReturnNode(Range(return_token.position.start_position, value.end_position), value)
end
